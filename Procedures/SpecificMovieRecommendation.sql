...
DELIMITER //
DROP PROCEDURE IF EXISTS SpecificMovieRecommendation;
CREATE PROCEDURE SpecificMovieRecommendation(IN P_YEAR INT, IN P_GENRE VARCHAR(20))
  BEGIN
    select t2.originalTitle,rt1.averageRating,t2.genres from 
      (select t1.tconst tconst, t1.originalTitle, t1.startYear, t1.type, t1.genres from TitleInfo t1 where t1.startYear = P_YEAR AND t1.type = 'movie'
