DELIMITER //
DROP PROCEDURE IF EXISTS GenericMovieRecommendation;
CREATE PROCEDURE GenericMovieRecommendation(IN P_YEAR INT)
BEGIN
    ...
    main_loop: LOOP
        ...
        SELECT t2.originalTitle, rt1.averageRating, t2.genres FROM (
            ...
            FROM (
                ...
                FROM TitleInfo t1
                WHERE t1.startYear = P_YEAR AND t1.type = 'movie' AND INSTR(LOWER(genres), LOWER(LANGNAME)) > 0
            ) ay
        ) t2
        INNER JOIN (
